/* eslint-disable max-len */
import { SVGProps } from 'react'

export const Cypress: React.FC<SVGProps<SVGSVGElement>> = (props: SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -15 270 270" preserveAspectRatio="xMidYMid" {...props}>
    <defs>
      <linearGradient x1="6.32%" y1="44.028%" x2="87.52%" y2="58.406%" id="a">
        <stop stopColor="#315C55" offset="0%" />
        <stop stopColor="#09293F" offset="100%" />
      </linearGradient>
      <linearGradient x1="54.298%" y1="27.262%" x2="50%" y2="100%" id="b">
        <stop stopColor="#4FB38D" offset="0%" />
        <stop stopColor="#315C55" offset="100%" />
      </linearGradient>
      <linearGradient x1="13.909%" y1="50%" x2="70.819%" y2="54.96%" id="c">
        <stop stopColor="#4FB38D" offset="0%" />
        <stop stopColor="#97DEC0" offset="100%" />
      </linearGradient>
      <linearGradient x1="53.86%" y1="30.892%" x2="38.388%" y2="95.794%" id="d">
        <stop stopColor="#97DEC0" offset="0%" />
        <stop stopColor="ivory" stopOpacity={0} offset="100%" />
        <stop stopColor="ivory" stopOpacity={0} offset="100%" />
      </linearGradient>
    </defs>
    <path
      d="m141.651 223.943-4.335 10.526c-.861 2.084-2.83 3.495-5.046 3.59-1.464.056-2.94.097-4.431.097-51.024-.027-96.169-36.113-107.355-85.836l-17.369 3.933c6.277 27.89 22.032 53.204 44.365 71.295 22.592 18.296 51.052 28.397 80.167 28.452h.26c1.71 0 3.406-.041 5.101-.11 9.136-.383 17.3-6.126 20.788-14.637l5.661-13.76-17.806-3.55Z"
      fill="url(#a)"
    />
    <path
      d="M4.54 161.914A128.496 128.496 0 0 1 0 128.007C0 75.516 32.863 27.52 81.782 8.593l6.414 16.652c-42.108 16.295-70.39 57.59-70.39 102.762a110.66 110.66 0 0 0 3.898 29.178L4.54 161.914Z"
      fill="url(#b)"
    />
    <path
      d="M225.693 77.298c-19.037-36.662-56.509-59.44-97.769-59.44-14.62 0-28.828 2.823-42.204 8.374L78.897 9.744C94.446 3.29 110.939 0 127.924 0c47.934 0 91.45 26.451 113.564 69.047l-15.795 8.25Z"
      fill="url(#c)"
    />
    <path
      d="M89.727 101.597c10.326 0 18.737 5.523 23.058 15.158l.342.754 17.34-5.907-.369-.891c-6.715-16.405-22.182-26.602-40.37-26.602-12.787 0-23.181 4.112-31.77 12.554-8.533 8.388-12.855 18.94-12.855 31.371 0 12.321 4.322 22.82 12.855 31.207 8.589 8.443 18.983 12.554 31.77 12.554 18.188 0 33.655-10.197 40.37-26.588l.37-.89-17.368-5.922-.329.782c-3.87 9.47-12.486 15.13-23.044 15.13-7.193 0-13.265-2.521-18.08-7.483-4.867-5.03-7.329-11.348-7.329-18.776 0-7.483 2.407-13.678 7.33-18.94 4.8-4.99 10.886-7.511 18.08-7.511Z"
      fill="#09293F"
    />
    <path
      fill="#09293F"
      d="m198.34 85.726-24.711 62.729-24.877-62.729h-20.35l34.888 85.617-24.822 60.385 17.792 3.55 61.31-149.552z"
    />
    <path
      d="M241.488 69.047C250.491 86.585 256 107.185 256 128.27c0 49.2-27.671 91.916-68.253 113.343l-8.768-15.548c35.19-18.352 59.227-55.258 59.227-97.795 0-18.46-4.513-35.691-12.513-50.973l15.795-8.251Z"
      fill="url(#d)"
    />
  </svg>
)
